...
class Styler:
    def __init__(self, mainTexFile='', bibFilename='', outputBibFile='', style_index=0):
        ...
        self.preamble = '\\begin{thebibliography}{9}'
    ...
    def GetInputFiles(self):
        ...
        try:
            f = open(self.mainTexFile, 'r', encoding='utf-8')
    ...
    def GetMainTexFileCites(self):
        ...
        try:
            f = open(self.filePath + self.mainTexFile, 'r', encoding='utf-8')
            s = f.read()  # read file to end
    ...
    def GetTexFileCites(self):
        ...
        try:
            for rawTexFile in self.aTexFiles:
                texFile = rawTexFile
                if ".tex" not in texFile:
                    texFile+=".tex"
                f = open(self.filePath + texFile, 'r', encoding='utf-8')
                s = f.read()  # read file to end
    ...
    def GetBibitems(self):
        ...
        try:
            with open(self.filePath + self.bibFilename, 'r', encoding='utf-8') as f:
            ...
            while s.find('\\bibitem') != -1:
                ...
                bibitem = bibitem.replace(" ", " ").replace("--", "–").replace(" /", "~/").replace(" –", "~–").replace(" :", "~:")
    ...
    def WriteBibFile(self):
        ...
        try:
            f = open(self.filePath + self.outputBibFile, 'w', encoding='utf-8')
